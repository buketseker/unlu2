...
class CurrencyRate(models.Model):
    ...
    banknot_selling_rate = fields.Float(digits=(12, 6), string="Efektif Satış")
...
class Currency(models.Model):
    ...
    #Döviz Satış
    rate = fields.Float(compute='_compute_current_rate', string='Döviz Satış', digits=(12, 6),
    ...
    banknot_selling_rate = fields.Float(digits=(12, 6), string="Efektif Satış", compute='compute_banknot_selling_rate')
    ...
    #                                         digits=(12, 6))
    inverse_banknot_selling_rate = fields.Float(compute="compute_inverse_rate_values", string="Ters Efektif Satış Kuru",
                                               digits=(12, 6))
    inverse_forex_buying_rate = fields.Float(compute="compute_inverse_rate_values", string="Ters Döviz Alış Kuru",
                                             digits=(12, 6))

    @api.multi
    @api.depends('rate', 'banknot_buying_rate', 'banknot_selling_rate', 'forex_buying_rate')
    def compute_inverse_rate_values(self):
        for currency in self:
            ...
            currency.inverse_banknot_selling_rate = 1 / currency.banknot_selling_rate or 0.0
    ...
    @api.multi
    @api.depends('rate_ids.rate')
    def compute_banknot_selling_rate(self):
        date = self._context.get('date') or fields.Date.today()
        company_id = self._context.get('company_id') or self.env['res.users']._get_company().id
        query = """SELECT c.id, (SELECT r.banknot_selling_rate FROM res_currency_rate r
                                          WHERE r.currency_id = c.id AND r.name <= %s
                                            AND (r.company_id IS NULL OR r.company_id = %s)
                                       ORDER BY r.company_id, r.name DESC
                                          LIMIT 1) AS banknot_selling_rate
                           FROM res_currency c
                           WHERE c.id IN %s"""
        self._cr.execute(query, (date, company_id, tuple(self.ids)))
        currency_rates = dict(self._cr.fetchall())
        for currency in self:
            currency.banknot_selling_rate = currency_rates.get(currency.id) or 1.0
    ...
    def compute(self, from_amount, to_currency, round=True, rate_type="default"):
        if rate_type == "default":
            ...
            if self == to_currency:
                ...
                elif rate_type == "Efektif Satış":
                    from_currency = self.with_env(self.env)
                    to_currency = to_currency.with_env(self.env)
                    rate = to_currency.rate / from_currency.banknot_selling_rate
