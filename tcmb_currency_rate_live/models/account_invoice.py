...
class AccountInvoice(models.Model):
    ...
    def update_currency_rate(self):
        for record in self:
            if record.currency_id and record.company_id.currency_id.id != record.currency_id.id:
                ...
                if currency_rate_ids:
                        ...
                        if currency_rates:
                            ...
                            if not last_id:
                                last_id = currency_rate_obj.create({'currency_id': currency.id,
                                                                    'rate': currency_rates["forex_selling"],
                                                                    'banknot_buying_rate': currency_rates[
                                                                        "banknote_buying"],
                                                                    'banknot_selling_rate': currency_rates[
                                                                        "banknote_selling"],
                    ...
                    if record.currency_rate_type_id:
                        ...
                        elif rate_type.name == 'Efektif Satış':
                            if last_id.banknot_selling_rate > 0.0:
                                record.currency_inverse_rate = 1 / last_id.banknot_selling_rate
